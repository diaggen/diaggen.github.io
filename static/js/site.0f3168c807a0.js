...
(() => {
  ...
  function initNavigation() {
    ...
    function sync() {
      scheduled = false;
      navigation.classList.toggle('is-stuck', window.scrollY > 0 && navigation.getBoundingClientRect().top <= 0.5);
      const probe = Math.max(navigation.offsetHeight + 32, Math.min(window.innerHeight * 0.22, 180));
      let active = 0;
