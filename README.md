...
- A horizontal section navigation bar starts below the Paper / arXiv / Code buttons and sticks 10px below the top while scrolling. When sticky, it becomes a rounded, pale-green floating bar with a soft shadow and a pill highlight for the current section. Links wrap into rows on smaller screens; section jumps account for the bar's height and top gap so headings stay visible. Reduced-motion preferences disable decorative transitions and smooth scrolling, while videos retain visible playback controls.
